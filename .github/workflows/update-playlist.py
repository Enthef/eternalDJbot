...
def get_my_liked_videos(y):
    ...
    page = None
    while True:
        resp = y.videos().list(part="id,snippet,contentDetails,status",
                               myRating="like", maxResults=50, pageToken=page).execute()
...
def get_videos_from_playlist(y, pid):
    ...
    page = None
    while True:
        pl = y.playlistItems().list(part="contentDetails", playlistId=pid, maxResults=50,
                                    pageToken=page).execute()
        ids = [i["contentDetails"]["videoId"] for i in pl.get("items",[])]
        if ids:
            v = y.videos().list(part="id,snippet,contentDetails,status", id=",".join(ids)).execute()
            vids += v.get("items",[])
        page = pl.get("nextPageToken")
        if not page: break
    return vids

def filter_music(items):
    OUT = []
    for it in items:
        ...
        # region restrictions (skip if blocked anywhere)
...
def current_playlist_video_ids(y, pid):
    ...
    page = None
    while True:
        resp = y.playlistItems().list(part="id,contentDetails,snippet",
                                      playlistId=pid, maxResults=50, pageToken=page).execute()
        for it in resp.get("items", []):
            vid = it["contentDetails"]["videoId"]
            ids.append(vid); map_id_to_item[vid] = it
        page = resp.get("nextPageToken")
        if not page: break
...
def main():
    ...
    # 2) seed playlists (e.g., enthef’s)
    for pid in SEED_PLAYLISTS:
        pool += get_videos_from_playlist(y, pid)

    # Filter, dedupe by videoId
    pool = filter_music(pool)
    uniq = {}
    for it in pool:
        uniq[it["id"]] = it
    pool = list(uniq.values())

    # Shuffle for variety & cap
    random.shuffle(pool)
